...
const app = express();
//const shortid = require("shortid"); changes id every single time? 
...
// 		const deletedUser = users.find((user) => user.id === request.params.id);
// 		deletedUser
// 			? response.status(500).json({ errorMessage: "no id." })
// 			: response.json(user);
// 	} else {
// 		response.status(404).json({ message: "no id" });
// 	}
